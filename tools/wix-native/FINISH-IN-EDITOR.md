...
*(Klub 10 was briefly recommended here. It regressed on section naming — see the
correction in `RUN-LOG.md`. Klub 8 is the most faithful build; close its gaps
with Aria rather than regenerating.)*
...
Page titles were written explicitly, so they no longer show the site name. Wix ignores the site name passed
